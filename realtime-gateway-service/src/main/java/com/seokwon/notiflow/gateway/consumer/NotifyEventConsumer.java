...
import com.seokwon.notiflow.gateway.event.NotifyPublishedEvent;

/**
 * notification-events 토픽을 구독해서 받은 즉시 STOMP로 브로드캐스트
 * 별도의 저장/가공 없이 그대로 전달만 함 - 가공은 이미 notify-service가 끝냈음.
...
public class NotifyEventConsumer {
    ...
    public void onNotifyPublished(NotifyPublishedEvent event) {
        messagingTemplate.convertAndSend(DESTINATION, event);
        log.debug("알림 브로드캐스트: notifyId={}", event.notifyId());
    }
}
